    ...
    return (
      <div className="flex gap-0.5">
        {[...Array(5)].map((_, i) => (
          <span
            key={i}
            className={`text-sm sm:text-base ${i < rating ? "text-yellow-500" : "text-gray-300"}`}
          >
            ★
          </span>
        ))}
          ...
          <div className="max-w-2xl mx-auto w-full px-4 py-4 sm:py-6">
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-6">
              <div className="bg-white p-4 sm:p-5 rounded-xl shadow-sm border border-gray-100">
                <p className="text-2xl sm:text-3xl font-bold text-center text-blue-600">
            ...
            <div className="space-y-3 sm:space-y-4">
              {isLoading ? (
                ...
                currentFeedback.map((feedback) => (
                  <div
                    key={feedback.id}
                    className={`bg-white p-4 sm:p-5 rounded-xl shadow-sm border space-y-3 transform transition hover:shadow-md ${
                      !feedback.read && activeTab === 'received' 
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 pt-3 border-t border-gray-100">
                      <p className="text-xs text-gray-400">
                        ...
                        <span className="hidden sm:inline">
                          {' '}at {new Date(feedback.created_at).toLocaleTimeString([], { 
                            hour: '2-digit', 
                            minute: '2-digit' 
                          })}
                        </span>
